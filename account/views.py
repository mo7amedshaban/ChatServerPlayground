...
def login_view(request):
    ...
    context['login_form'] = form  # for validation
...
def account_search_view(request, *args, **kwargs):
    ...
    if request.method == "GET":
        search_query = request.GET.get("q")  # this exact in api    ?search=
        if len(search_query) > 0:
            # icontains  search for lowercase or uppercase
            # contains   search only word detected
...
# ----------> Two function for use crop image <---------------
